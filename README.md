...
- **Fills**: solid color, linear gradient with angle, radial gradient with a movable centre and adjustable spread, and a conic sweep. Any gradient takes up to six extra colors between its two ends, each positioned where you want it. Discord's default role color palette is there as one-click swatches.
- **Undo and redo**: every change is reversible, from the header buttons or Ctrl+Z and Ctrl+Shift+Z. Dragging a slider collapses into a single step.
- **Unlimited layers**: stack up to 24 marks on one icon and combine anything with anything. Text next to an emoji, a symbol over a shape, two images, a pale plate behind a crown. Each layer has its own size, position, rotation, opacity, flip, drop shadow, blend mode and a switch for whether it is clipped to the background silhouette. A layer can also be masked to another layer's silhouette. Reorder, hide, duplicate and delete from the Layers tab.
...
- **Per-layer effects**: a colored glow, an outline traced round the mark, and a recolor. All three work on any layer, including uploaded pictures and emoji, so recoloring is the one way to change an emoji's colors.
- **Blend modes**: the sixteen canvas blend modes, plus **erase**, which punches a layer out of everything below it, and **stencil**, which keeps what is below only where the layer covers it. Both stay inside the icon silhouette.
- **Plate effects**: border, drop shadow, glossy highlight, content shadow, and full placement control (size, offset, rotation, opacity).
